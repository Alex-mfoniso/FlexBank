...
\`\`\``
    },
    {
      ...
      details: `Ricarut returns standard HTTP status codes. Failures include a detailed diagnostic JSON envelope explaining parameter faults and business validation violations:
...
* **\`422 Unprocessable\`**: Business logical constraints violated (e.g. frozen account, insufficient funds).`
    },
    {
      ...
      details: "Customers form the identity layer of your Ricarut project ledger. Associate customer records with multi-currency accounts to trace compliance, track assets, and route transactional notifications.",
      endpoints: [
        {
          id: "post-customers",
    ...
    },
    {
      ...
      details: "Whenever a transfer completes, Ricarut writes ledger transactions. Transactions operate as read-only journals detailing exact credits and debits recorded on account nodes.",
      endpoints: [
        {
          id: "get-transactions",
      ...
      endpoints: [
        {
          ...
          defaultPayload: JSON.stringify({
            url: "https://yourserver.com/webhooks/ricarut"
          }, null, 2),
    ...
    },
    {
      ...
      details: "Ricarut exposes dedicated test-mode-only helper endpoints. These tools let developers simulate credit actions, settle transfers, trigger webhook alerts, or completely wipe test databases.",
      endpoints: [
        {
          id: "post-fund",
  ...
  const getSectionOutline = (docId: string) => {
    ...
    if (docId === "overview") {
      return [
        ...
        { id: "core-capabilities", label: "What is Ricarut?" }
      ];
    }
    if (docId === "quickstart") {
      return [
      ...
      {isPublicView && (
        <div className="max-w-7xl mx-auto flex items-center justify-between pb-6 border-b border-neutral-900 mb-8">
          <Link to="/" className="flex items-center space-x-3 group">
            ...
            <div>
              <span className="text-sm font-black text-white tracking-wider block leading-none">RICARUT</span>
